...
BACKUP_CMD="mongodump --out "'$BACKUP_PATH'/'${BACKUP_NAME}'" --host ${MONGODB_HOST} --port ${MONGODB_PORT} ${USER_STR}${PASS_STR}${DB_STR} ${EXTRA_OPTS}"
...
if [ -n "\${MAX_BACKUPS}" ]; then
    while [ \$(ls \${BACKUP_PATH} -N1 | wc -l) -gt \${MAX_BACKUPS} ];
    do
        BACKUP_TO_BE_DELETED=\$(ls \${BACKUP_PATH} -N1 | sort | head -n 1)
        echo "   Deleting backup \${BACKUP_TO_BE_DELETED}"
        rm -rf \${BACKUP_PATH}/\${BACKUP_TO_BE_DELETED}
    done
fi
echo "=> Backup done"
EOF
